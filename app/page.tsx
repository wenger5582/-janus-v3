...
export default function Home() {
  ...
  async function translateText(text: string, target: string, source: string) {
    ...
    const data = await res.json(); return data.translated || text
  }
  useEffect(() => {
    if (!selected) return
    setTTitle(selected.title); setTDesc(selected.description)
    if (lang === 'es' && (selected.source === 'CHILE' || selected.source === 'ESPAÑA')) return
    if (lang === 'en' && (selected.source === 'USA' || selected.source === 'UK')) return
    ;(async () => { setLoadingTrans(true); const [tt, td] = await Promise.all([translateText(selected.title, lang, selected.source), translateText(selected.description, lang, selected.source)]); setTTitle(tt); setTDesc(td); setLoadingTrans(false) })()
  }, [lang, selected])
  ...
  return (
    <div style={{ minHeight: '100vh', background: '#050505', color: 'white', paddingBottom: 40 }}>
      <h1 style={{ textAlign: 'center', fontSize: 42, fontWeight: 900, letterSpacing: 10, padding: '32px 0 20px', fontFamily: 'Times New Roman, serif' }}>JANUS V3</h1>
      <div style={{ maxWidth: 700, margin: '0 auto', padding: '0 16px', display: 'grid', gap: 14 }}>
        {news.map((n: any) => (
          <div key={n.link} onClick={() => setSelected(n)} style={{ background: '#151515', border: '1px solid #232323', borderRadius: 24, padding: '20px 22px', cursor: 'pointer' }}>
            <div style={{ fontSize: 11, color: '#C7A46B', fontWeight: 900, letterSpacing: 2, marginBottom: 8 }}>{n.source}</div>
            <div style={{ fontSize: 19, fontWeight: 700, lineHeight: 1.3 }}>{n.title}</div>
          </div>
        ))}
      </div>
      {selected && (
            ...
            <div style={{ padding: 20 }}>
              <div style={{ display: 'flex', gap: 12, marginBottom: 28 }}>
                <button onClick={() => setLang('es')} style={{ flex: 1, padding: '14px 8px', borderRadius: 28, background: lang === 'es'? '#C7A46B' : '#1E1E1E', color: lang === 'es'? 'black' : 'white', border: 'none', display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 4 }}>
                  <span style={{ fontSize: 22 }}>🇪🇸</span><span style={{ fontSize: 14, fontWeight: 900 }}>Español</span>
                </button>
                <button onClick={() => setLang('en')} style={{ flex: 1, padding: '14px 8px', borderRadius: 28, background: lang === 'en'? '#C7A46B' : '#1E1E1E', color: lang === 'en'? 'black' : 'white', border: 'none', display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 4 }}>
                  <span style={{ fontSize: 22 }}>🇬🇧</span><span style={{ fontSize: 14, fontWeight: 900 }}>English</span>
                </button>
                <button onClick={() => setLang('ru')} style={{ flex: 1, padding: '14px 8px', borderRadius: 28, background: lang === 'ru'? '#C7A46B' : '#1E1E1E', color: lang === 'ru'? 'black' : 'white', border: 'none', display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 4 }}>
                  <span style={{ fontSize: 22 }}>🇷🇺</span><span style={{ fontSize: 14, fontWeight: 900 }}>Русский</span>
                </button>
              </div>
              <h2 style={{ fontFamily: 'Georgia, serif', fontSize: 34, fontWeight: 900, lineHeight: 1.15, marginBottom: 20 }}>{loadingTrans? 'Traduciendo...' : tTitle}</h2>
              ...
              <a href={selected.link} target="_blank" style={{ display: 'block', marginTop: 22, background: '#C7A46B', color: 'black', textAlign: 'center', padding: 18, borderRadius: 14, fontWeight: 900, textDecoration: 'none' }}>Leer fuente original</a>
              <button onClick={() => setSelected(null)} style={{ width: '100%', marginTop: 12, background: '#1E1E1E', color: 'white', padding: 16, borderRadius: 14, border: 'none', fontWeight: 700 }}>Cerrar</button>
            </div>
          </div>
        </div>
      )}
