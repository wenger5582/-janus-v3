...
export default function Page(){
 ...
 return(
  <div style={{background:"#000",minHeight:"100vh",color:"#FCEE0A",padding:12}}>
    ...
    <div style={{fontSize:10,color:"#888"}}>{all.length===0?"Cargando...":`${f.length} noticias`} {sel?`- ${sel}`:""}</div>
    <div style={{display:"flex",gap:6,flexWrap:"wrap",margin:"12px 0"}}>
     {["WSJ","REUTERS","BBC","CNN","BLOOMBERG","AP"].map(c=>
       <button key={c} onClick={()=>setSel(c===sel?null:c)} style={{background:sel===c?"#fcee0a":"#111",color:sel===c?"#000":"#fff",borderRadius:20,padding:"7px 14px",fontWeight:900,border:"1px solid #333"}}>{c}</button>
     )}
    ...
    <div style={{display:"grid",gridTemplateColumns:"repeat(2,1fr)",gap:8}}>
     {f.slice(0,100).map((n:any)=>
       <div key={n.id} style={{background:"#0f0f00",border:"1px solid #222",borderLeft:"3px solid #fcee0a",borderRadius:10,overflow:"hidden"}}>
        ...
        <div style={{padding:6,fontSize:11,color:"#fff",fontWeight:700}}>{n.title?.slice(0,80)||"Sin titulo"}</div>
